...
#### P0-1. runtime health / status mismatch RCA + fix（置顶，未解决前不降级）
目标：
- 先定位并修复 runtime health 与外部 status 不一致问题
- 在问题关闭前，持续作为主队列第一优先级

要求：
- 明确 RCA、影响面、临时绕行与最终修复口径
- 禁止把 unknown / stale / partial 状态包装成 healthy / ready
- 修复后再恢复常规报告闭环优先级

#### P0-2. 早报一主三辅的 end-to-end 生成闭环
目标：
- 在 09:15 前稳定生成：
  - MAIN early
  - macro early
...
- runtime health/status mismatch 的 RCA+fix 现为主队列置顶项，未解决前不降级
